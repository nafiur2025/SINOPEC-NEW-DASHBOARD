...
export default function TabbedBreakdown({ ads }: Props) {
  ...
  const [metric, setMetric] = React.useState<(typeof metrics)[number]['key']>('cpm_bdt')

  const filtered = ads.filter(a => a.level === level)

  const groups = new Map<string, any[]>()
  for (const r of filtered) {
    const key = (level==='campaign' ? r.campaign_name : level==='adset' ? r.adset_name : r.ad_name) || 'Unknown'
  ...
  return (
    <div className="card">
      <div className="card-header">Breakdown by {level}</div>
      <div className="card-body space-y-4">
        <div className="flex flex-wrap gap-2">
          ...
          <select className="border rounded-lg px-3 py-1 text-sm ml-auto" value={metric} onChange={e=>setMetric(e.target.value as any)}>
